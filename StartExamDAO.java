...
public class StartExamDAO {
    ...
    public int startSaveResult(String studentID,int lessonID){
    	...
    	String sql="INSERT INTO tb_stuResult (id,stuId,whichLesson,resSingle,resMore,joinTime) values(null,'"+studentID+"','"+lesson+"',0,0,now())";
    ...
    public int saveResult(String studentID,int lessonID,int resSingle,int resMore){
    	String lesson=((LessonForm)lessonDAO.query(lessonID).get(0)).getName();
    	String sql="UPDATE tb_stuResult set resSingle="+resSingle+",resMore="+resMore+",resTotal="+(resSingle+resMore)+" WHERE stuId='"+studentID+"' AND whichLesson='"+lesson+"'";
